(function () {
  ...
  const turnLoop = (now) => {
    const e = engine();
    const dt = Math.min(30, now - previous || 16);
    previous = now;
    ...
    if (e && e.p && typeof e.mobileLook === 'number') {
      ...
      if (Math.abs(turnAmount) > 0.00002) {
        ...
        p.dirX = oldDirX * c - oldDirY * s;
        p.dirY = oldDirX * s + oldDirY * c;
        p.planeX = oldPlaneX * c - oldPlaneY * s;
        p.planeY = oldPlaneX * s + oldPlaneY * c;
      }
    }
  ...
  window.weaponProjectile = function (weapon, color) {
    ...
    shots.push({
      x1: centerX - 10,
      y1: centerY + 10,
      x2: centerX + (Math.random() - 0.5) * spread,
      y2: centerY + (Math.random() - 0.5) * spread,
      t: 0,
      color: color || '#ff0844',
    ...
    const draw = () => {
      ...
      for (let i = shots.length - 1; i >= 0; i--) {
        ...
        ctx.shadowBlur = s.weapon === 'SHOTGUN' ? 26 : 14;
